...
func TestCreateVersionInfo(t *testing.T) {
	...
	t.Run("creates a VersionInfo struct", func(t *testing.T) {
		...
		require.Equal(t, result.StringFileInfo.Comments, "Equipped Agent")
		require.Equal(t, result.StringFileInfo.CompanyName, "Equipped Inc.")
		require.Equal(t, result.StringFileInfo.FileDescription, "Equipped Agent installer")
		require.Equal(t, result.StringFileInfo.FileVersion, "1.2.3.0")
		require.Equal(t, result.StringFileInfo.LegalCopyright, fmt.Sprintf("%d Equipped Inc.", time.Now().Year()))
		require.Equal(t, result.StringFileInfo.ProductName, "Equipped Agent")
